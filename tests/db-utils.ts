...
import { type DB } from '#app/types/kysely-types.js' // this is the Database interface we defined earlier
...
export async function cleanupDb(db: Kysely<DB>) {
	const tables = await sql<{
		name: string
	}>`SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE 'kysely_migration%';`.execute(
		db,
	)
	...
	await db.transaction().execute(async trx => {
		...
		await Promise.all(
			tables.rows.map(({ name }) => sql`DELETE from "${name}"`.execute(trx)),
		)
