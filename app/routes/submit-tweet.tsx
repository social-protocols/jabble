...
import { type OEmbedResponse } from '#app/integrations/common.ts'
import { matchIntegration } from '#app/integrations/integrations.client.ts'
...
export default function SubmitContentPage() {
	...
	useEffect(() => {
		...
		if (integration !== undefined) {
			const siteName = integration.siteName

			setIsValidUrl(true)
			setFetchError(null)
			setEmbedHtml(undefined) // Reset embedHtml when URL changes
			setContentType(siteName)
	...
	useEffect(() => {
		if (embedHtml) {
			...
			if (contentType === 'Twitter') {
				...
				console.log('Setting ocntent loaded', contentType)
				setIsContentLoaded(true)
			}
		}
	...
	return (
		<div className="mb-4 flex flex-col space-y-2 rounded-xl border-2 border-solid border-gray-200 p-4 text-sm dark:border-gray-700">
				...
				<div className="flex items-center justify-between">
					{isValidUrl ? (
						fetchError ? (
							...
							<div style={{ color: 'green' }}>✓ Valid {contentType} URL</div>
						)
					) : (
			...
			{isFetching && <div>Fetching {contentType} content...</div>}
			{embedHtml && (
