...
export const action = async (args: ActionFunctionArgs) => {
  ...
  const fallacy = await fallacyDetection(postData.content)

  await db.transaction().execute(
    async trx =>
      await sql`
        insert into fallacies (postId, detection)
        values (${postId}, ${JSON.stringify(fallacy)})
      `.execute(trx)
  )
...
const FallacySchema = z.object({
  ...
  detected_fallacies: z.array(
    z.object({
      ...
      analysis: z.string().describe("Explain why the given fallacy is present and how this leads to a wrong conclusion. Use simple language. Be concise. Write in the language of the content you analyze."),
      probability: z.number().describe("Probability of the fallacy being present based on the analysis (between 0.0 and 1.0).")
    }).strict()
  ).describe("The detected fallacies. If in doubt, conduct the analysis as well.")
}).strict()

type Fallacy = z.infer<typeof FallacySchema>

async function fallacyDetection(content: string): Promise<Fallacy> {
...
E₁ is part of E, E has property P. Therefore, E₁ has property P.
    ...
    response_format: zodResponseFormat(FallacySchema, "event"),
  });
