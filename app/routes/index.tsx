...
export function TopLevelPost({
	...
	const effect = Math.exp(Math.random())
	...
	return (
		...
		>
			<div className="postteaser flex w-full min-w-0 flex-col">
				<div className="mt-1 text-left text-sm opacity-50"></div>
				<div className="mt-1 flex text-sm opacity-50">
					{/*
          <Link className="font-bold" to={`/tags/${post.tag}`}>
            #{post.tag}
          </Link>
          */}
					<span className="mr-auto">no effect</span>
					{/*<span className="ml-auto">{ageString}</span>*/}
				</div>
				<PostContent
          ...
          */}
					<Link to={`/tags/${post.tag}/posts/${post.id}`} className="hyperlink">
						{nRepliesString}
					</Link>
				</div>
			</div>
