...
export function StatsPostAnnotation({annotation}: {annotation: string}) {
	return (
		<p style={{color: 'grey', fontSize: '0.7rem', marginBottom: '10px'}}>{annotation}</p>
	)
}
...
export function EffectStrengthConnectorLine(
	...
	const borderWidth = effectStrength > 0.1 ? effectStrength > 0.5 ?
		'border-l-8' : 'border-l-4' : 'border-l-2'

	const upvoteRatioChange = informedProb - uninformedProb

	const effectDescription = upvoteRatioChange! >= 0 ?
		'causes people to upvote more' : 'causes people to upvote less'

	return (
		<div className={borderWidth + ' my-2 mx-5 p-2 h-10 border-gray-500 text-sm'}>
			{effectDescription}
		</div>
	)
